...
SYSTEM_PROMPT = (
    ...
    "`beneficiario_ingresos_brutos_anuales` (los ingresos brutos de TODAS las "
    "fuentes antes de impuestos y deducciones: operacionales MÁS no "
    "operacionales u otros ingresos, si el documento los separa; NO la utilidad "
    "ni un margen). Si las cifras vienen expresadas en miles o millones, "
    "conviértelas a pesos.\n"
    "- Periodo de los estados financieros: extrae el HECHO, nunca la "
    "conclusión. Copia la fecha de corte del encabezado ('a 31 de diciembre de "
    "2025', 'acumulado a 30 de junio') en `fecha_balance_dia`, `fecha_balance_mes` "
    "y `fecha_balance_anio`, y en `estados_financieros_meses_cubiertos` cuántos "
    "meses cubre el estado de resultados (12 si va del 1 de enero al 31 de "
    "diciembre; 6 si es un acumulado a 30 de junio; null si el documento no lo "
    "dice). NUNCA anualices ni proyectes una cifra parcial, y NUNCA descartes "
    "las cifras por el periodo que cubran: transcríbelas como están — si el "
    "periodo no sirve, lo detecta el sistema con la fecha de corte.\n"
